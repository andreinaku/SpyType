...
class NodeInfer:
    def __init__(self, _in: AbsState, _code: str):
        self.out_as = AbsState()
        self.in_as = _in
        self.code = _code
...
def dump_roundineqs(round_ineqs: dict[int, dict[int, tuple[AbsState]]]):
    writestr = ''
...
def analyze(blockinfo: dict, init: AbsState, dataclass, cfg: CFG, dbg=False):
    Trounds: list[dict[int, NodeInfer]]
    Trounds = []
    round_ineqs: dict[int, dict[int, tuple[AbsState]]] = dict()
    init_TI = dict()
    blocks = deepcopy(list(blockinfo.keys()))
    for b in blocks:
        ni = NodeInfer(AbsState(), '')
