...
class ScreenForTest(object):
    ...
    def getRowsForPastScreen(self, pastScreen):
        return self.getRows(#screen=
                            self.pastScreens[pastScreen])

    def getRowsWithAttributesForPastScreen(self, pastScreen):
        return self.getRowsWithAttributes(#screen=
                                          self.pastScreens[pastScreen])
    ...
    def getRowsWithAttributesForPastScreens(self, pastScreens):
        ...
        pages = [self.getRowsWithAttributes(screen=self.pastScreens[screenIndex]) for screenIndex in pastScreens]

        # join the pages together into one stream
        # cheating
        # lines, attributes = zip(*pages)
        lines = [x for (x, y) in pages]
        attributes = [y for (x, y) in pages]
