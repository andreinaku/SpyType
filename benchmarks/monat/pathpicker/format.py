...
class LineMatch(object):
    ...
    def updateDecoratedMatch(self, maxLen=None):
        ...
        self.decoratedMatch = FormattedText(
            # FormattedText.getSequenceForAttributes(*attributes) CHEATING
            FormattedText.getSequenceForAttributes(attributes[0], attributes[1], attributes[2])
            +
            plainText)
    ...
    def output(self, printer):
        ...
        # cheating *soFar replaced into soFar[0], soFar[1]
        soFar = self.printUpTo(self.beforeText, printer, y, soFar[0], soFar[1])
        soFar = self.printUpTo(self.decoratedMatch, printer, y, soFar[0], soFar[1])
        soFar = self.printUpTo(self.afterText, printer, y, soFar[0], soFar[1])
