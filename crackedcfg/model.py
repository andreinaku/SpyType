...
class Block(object):
    ...
    __slots__ = ["id", "statements", "func_calls", "predecessors", "exits", "new_id"]
...
class CFG(object):
    ...
    def __init__(self, name: str, params: list, asynchr=False):
        ...
        self.varset = set()
        # CFG dictionary for inference use
        self.cfgdict = dict()
        # converging block id
        self.converging_id = -1
        # parameters (if any)
        self.params = copy.deepcopy(params)
        # class ASTs
        self.class_asts = dict()
        # function ASTs
        self.func_asts = dict()
    ...
    def _build_visual(self, format='pdf', calls=True):
        graph = gv.Digraph(name='cluster'+self.name, format=format,
                           graph_attr={'label': self.name}, engine='dot')
    ...
    def get_varset(self):
        full_varset = copy.deepcopy(self.varset)
        full_varset.add('$return')
        return full_varset
